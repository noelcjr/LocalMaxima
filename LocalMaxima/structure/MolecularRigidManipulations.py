...
class structure_manipulations(object):
    ...
    def output_structure(self, indx, out_path='\0'):
        """
        Outputs an aligned PDB file. When there is no path, PDBIO() denies 
        permission. This could be because it might not allow to overwrite 
        files it just loaded. As a solution, the aligned structure name
        ...
        if out_path == '\0':
            file_path = "./"
            file_name = self.structures[indx].structure.get_id()+"_aligned.pdb"
            self.structures[indx].save_structure(file_path+file_name)
        else:
