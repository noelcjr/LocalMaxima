...
class mod_structure(object):
    def __init__(self, str_path):
        ...
        #                                              'params/peptides.pdb')
        # This class is similar to the one in structure3D.py. They can be
        # joined through a function that is more abstract, and from which these
        # two classes extend.
        self.LS = []
        self.mdls = {}
        ...
        self.pep_path = "/home/noel/Code/Local_Maxima/LocalMaxima/ThirdPartySoftware/charmm/params/peptides.pdb"
        self.s_pep = struct.PDBParser(QUIET=True).get_structure(
            'peptides', self.pep_path)
    ...
    def min_max_resid_name(self):
        ...
        """
        for g in self.structure.get_models():
            chns = {}
    ...
    def add_CTER_dir(self, chn_id, linker, position):
        ...
        if not gap_in_pos[0]:
            ...
            fixed_atoms = [i[3] for i in SE_F_M.atom_list(self.structure)[0]]
            ...
            for i in self.new_s.get_models():
                ...
                for j in i.get_chains():
                    if j.get_id() == chn_id:
                        for x in self.structure.get_models():
                            residues_per_model = []
    ...
    def add_NTER_dir(self, chn_id, linker, position):
        ...
        if not gap_in_pos[0]:
            ...
            for j in self.structure.get_models():
    ...
    def remove_from_se(self, remove, optionsout):
        rSE = SE.structural_expression(remove)
        remove_atoms = rSE.atom_list(self.structure)
        keys_rm = {}
